...
{
	Eigen::Matrix<double, 21, 1> res = Eigen::Matrix<double, 21, 1>::Zero();  // 初始化
	vect3 omega;
	...
	for(int i = 0; i < 3; i++ ){
		res(i) = omega[i];
		res(i + 3) =  s.vel[i];
		res(i + 6) = a_inertial[i] + s.grav[i]; // 赋值
	}
	return res;
}
...
Eigen::Matrix<double, 21, 15> df_dw(state_ikfom &s, const input_ikfom &in, const Eigen::Matrix3d & forward_kinematics_rotation)
{
